...
app = FastAPI(title="SLAM Live Job Discovery", version="8.0.0")
...
ALIASES = {
 ...
 "br":["brazil","sao paulo","rio de janeiro","brasilia"],"ae":["uae","united arab emirates","dubai","abu dhabi"]}
...
def role_family(profile:dict[str,Any])->tuple[str,list[str]]:
    ...
    groups=[
      (["data scientist","data science","machine learning","ml engineer","ai engineer","artificial intelligence"],"Data & AI",["data scientist","machine learning engineer","ai engineer","data analyst"]),
      (["devops","sre","site reliability","cloud engineer","platform engineer"],"Cloud & DevOps",["devops engineer","cloud engineer","site reliability engineer","platform engineer"]),
      (["frontend","front-end","react developer","ui developer"],"Frontend Engineering",["frontend developer","frontend engineer","react developer","web developer"]),
      (["backend","back-end","api developer"],"Backend Engineering",["backend developer","backend engineer","api developer","software engineer"]),
      (["full stack","full-stack"],"Full Stack Engineering",["full stack developer","full stack engineer","software engineer","web developer"]),
      (["mobile","android","ios","flutter"],"Mobile Engineering",["mobile developer","android developer","ios developer","flutter developer"]),
      (["security","cyber","infosec"],"Cybersecurity",["cybersecurity analyst","security engineer","information security"]),
      (["qa","quality assurance","test engineer","automation tester"],"Quality Engineering",["qa engineer","test engineer","automation engineer","software engineer"]),
      (["product manager","product owner"],"Product",["product manager","product owner","technical product manager"])]
...
def build_queries(req:JobSearchRequest)->tuple[str,list[str],str]:
    ...
    if requested and requested.lower() not in presets:return requested,list(dict.fromkeys([requested]+roles[:2])),family
    return roles[0],roles[:3],family

def adzuna_location(v:Any)->str:
    if not isinstance(v,dict):return clean(v)
    area=v.get("area"); display=clean(v.get("display_name"))
    if isinstance(area,list) and area:
        parts=[clean(x) for x in area if clean(x)]
        if display and all(norm(x) not in norm(display) for x in parts):parts=[*reversed(parts)]
        return ", ".join(parts)
    return display
...
def is_fresh(date_text:str,max_days:int=60)->bool:
    try:
        age=(datetime.now(timezone.utc).date()-datetime.strptime(date_text[:10],"%Y-%m-%d").date()).days
...
def profile_roles(profile:dict[str,Any])->list[str]:
    vals=profile.get("targetRoles") or ([profile.get("currentRole")] if profile.get("currentRole") else [])
...
def location_fit(code:str,requested:str,job_location:str,description:str,remote:bool,feed_country:str="")->tuple[bool,str,int]:
    ...
    if remote:
        if feed_country==code:return True,"Remote feed is scoped to the candidate region",95
...
def match_score(profile:dict[str,Any],title:str,required:list[str],location_score:int,minimum_years:int)->dict[str,Any]:
    skills={norm(x) for x in (profile.get("skills",[])+profile.get("technologies",[]))};matched=[x for x in required if norm(x) in skills];missing=[x for x in required if norm(x) not in skills]
    exp=float(profile.get("yearsOfExperience") or 0);exp_score=100 if minimum_years<=0 else max(0,min(100,round(100-max(0,minimum_years-exp)*25)))
    targets=profile_roles(profile);role_score=100 if targets and any(r in norm(title) or norm(title) in r for r in targets) else 65;skill_score=round(100*len(matched)/len(required)) if required else 55
    total=round(skill_score*.5+exp_score*.2+role_score*.2+location_score*.1)
...
def normalize_job(source:str,item:dict[str,Any],profile:dict[str,Any],code:str,requested:str,feed_country:str="")->dict[str,Any]|None:
    title=clean(item.get("title") or item.get("jobTitle") or item.get("position"));cv=item.get("company") or item.get("companyName") or item.get("company_name");company=clean(cv.get("display_name") if isinstance(cv,dict) else cv)
    location=adzuna_location(item.get("location")) if source=="Adzuna" else clean(item.get("jobGeo") or item.get("jobLocation") or item.get("location") or item.get("locationName") or item.get("geo") or "Remote")
    description=clean(item.get("jobDescription") or item.get("description") or item.get("descriptionText") or item.get("jobExcerpt"));url=str(item.get("redirect_url") or item.get("apply_url") or item.get("url") or "").strip();date=parse_date(item.get("created") or item.get("pubDate") or item.get("date") or item.get("published") or item.get("created_at"));remote=bool(item.get("remote")) or source in {"Jobicy","Remote OK"} or "remote" in norm(f"{title} {location}")
...
async def fetch_remoteok(client:httpx.AsyncClient,profile:dict[str,Any],query:str)->list[dict[str,Any]]:
    skills=[norm(x).replace(" ","-") for x in (profile.get("skills",[])+profile.get("technologies",[])) if str(x).strip()];role=norm(query).replace(" ","-") if query else "software-engineer";tags=list(dict.fromkeys([role]+skills[:1]))
    data=await get_json(client,"https://remoteok.com/api",{"tags":",".join(tags)});return [x for x in data if isinstance(x,dict) and x.get("position")] if isinstance(data,list) else []
...
async def search_jobs(req:JobSearchRequest):
    ...
    visible,queries,_=build_queries(req);requested=req.location or req.country;timeout=httpx.Timeout(14.0,connect=5.0)
    ...
    all_jobs=[]
    for source,items,feed_country in [("Adzuna",adzuna_items,""),("Jobicy",jobicy_items,code),("Remote OK",remoteok_items,"")]:
        for item in items:
            job=normalize_job(source,item,req.profile,code,requested,feed_country)
