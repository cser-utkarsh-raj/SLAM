...
const safeParse = (key:string, fallback:any) => { try { return JSON.parse(localStorage.getItem(key) || 'null') ?? fallback; } catch { return fallback; } };
...
export default function App() {
  ...
  useEffect(()=>{
    const role = userProfile.targetRoles.find(Boolean) || userProfile.currentRole || 'software engineer';
    const country = userProfile.country.trim();
    const query = country ? `${role} ${country}` : role;
    const controller = new AbortController();
    fetch(`${API}/api/jobs/search`,{method:'POST',headers:{'Content-Type':'application/json'},signal:controller.signal,body:JSON.stringify({query,location:userProfile.location||'',country,remote:true,limit:30,profile:userProfile})})
      .then(async r=>{const raw=await r.text();let data:any={};try{data=raw?JSON.parse(raw):{}}catch{throw new Error(raw.slice(0,200)||'Job service returned invalid data.')}if(!r.ok)throw new Error(data.detail||data.error||`Job search failed (${r.status})`);return data;})
  ...
  const prepare=(job:JobPosting)=>setActiveTab('discover');
  ...
  return <div className="min-h-screen bg-[#050505] text-zinc-100 flex flex-col font-sans selection:bg-yellow-400 selection:text-black"><Navbar activeTab={activeTab} setActiveTab={setActiveTab} userProfile={userProfile}/><main className="flex-1 w-full pb-16">
