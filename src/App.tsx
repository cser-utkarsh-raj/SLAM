...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = AuthService.onAuthStateChanged(async user => {
      ...
      const nextProfile = profile || userProfile;
      if (!hasProfile(nextProfile)) setActiveTab('onboarding');
    });
    return unsubscribe;
  }, []);
  ...
  useEffect(() => { if (userProfile.country) setCountryQuery(v => v || userProfile.country); if (!searchQuery && userProfile.targetRoles[0]) setSearchQuery(userProfile.targetRoles[0]); }, [userProfile.country, userProfile.targetRoles]);

  const performJobSearch = useCallback(async () => {
    if (!authUser || !hasProfile(userProfile)) return;
    setIsSearching(true);
    try {
      const response = await fetch(`${API}/api/jobs/search`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ query:searchQuery || userProfile.targetRoles[0] || 'software engineer', location:userProfile.location, country:countryQuery || userProfile.country, remote:remoteOnly, limit:30, profile:userProfile }) });
      const raw = await response.text();
      let data:any={}; try { data=raw ? JSON.parse(raw) : {}; } catch { throw new Error('Job service returned invalid data.'); }
      if (!response.ok) throw new Error(data.error || data.detail || `Job discovery failed (${response.status}).`);
      setJobs(Array.isArray(data.jobs) ? data.jobs : []);
    } catch (error) { console.warn('Job search:', error); setJobs([]); }
    finally { setIsSearching(false); }
  }, [authUser, userProfile, searchQuery, countryQuery, remoteOnly]);

  useEffect(() => { if (authUser && hasProfile(userProfile) && activeTab === 'discover') void performJobSearch(); }, [authUser, userProfile.country, userProfile.targetRoles.join('|')]);
  ...
  if (showLanding && !authUser) return <div className="min-h-screen bg-[#050505] flex flex-col"><LandingPage onGetStarted={()=>setIsAuthModalOpen(true)} onSignIn={()=>setIsAuthModalOpen(true)} /><DotFooter /><AuthModal isOpen={isAuthModalOpen} onClose={()=>setIsAuthModalOpen(false)} onSuccess={u=>{setAuthUser(u);setShowLanding(false);setIsAuthModalOpen(false);setActiveTab('onboarding');}} /></div>;

  return <div className="min-h-screen bg-[#050505] text-zinc-100 flex flex-col selection:bg-yellow-400 selection:text-black">
    <Navbar activeTab={activeTab} setActiveTab={setActiveTab} userProfile={userProfile} authUser={authUser} onOpenAuth={()=>setIsAuthModalOpen(true)} onSignOut={signOut} onOpenSlamPlus={()=>setIsSlamPlusModalOpen(true)} isSubscribed={isSubscribed} />
    ...
    <DotFooter />
    <AuthModal isOpen={isAuthModalOpen} onClose={()=>setIsAuthModalOpen(false)} onSuccess={u=>{setAuthUser(u);setShowLanding(false);setIsAuthModalOpen(false);if(!hasProfile(userProfile))setActiveTab('onboarding');}} />
    <SlamPlusModal isOpen={isSlamPlusModalOpen} onClose={()=>setIsSlamPlusModalOpen(false)} isSubscribed={isSubscribed} onToggleSubscription={()=>setIsSubscribed(false)} />
  </div>;
}
