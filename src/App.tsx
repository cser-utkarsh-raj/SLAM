...
function profileSearchQuery(profile:UserProfile) { return (profile.targetRoles?.[0] || profile.currentRole || 'Software & Technology').trim().slice(0,120); }
...
export default function App() {
  ...
  useEffect(()=>{if(!locationQuery&&userProfile.country)setLocationQuery(userProfile.country); if(!searchQuery&&hasProfile(userProfile))setSearchQuery(profileSearchQuery(userProfile));},[userProfile.country,userProfile.targetRoles,userProfile.currentRole,locationQuery,searchQuery]);
  ...
  const openSignIn=()=>{setAuthModalMode('signin');setIsAuthModalOpen(true);}; const openSignUp=()=>{setAuthModalMode('signup');setIsAuthModalOpen(true);};
