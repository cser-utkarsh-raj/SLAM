...
export default function App() {
  ...
  const performJobSearch=useCallback(async()=>{
    ...
    try{
      const response=await fetch(`${API}/api/jobs/search`,{
        ...
        body:JSON.stringify({query:searchQuery||profileSearchQuery(userProfile),location:userProfile.location,country:countryQuery||userProfile.country,remote:remoteOnly,limit:30,profile:userProfile})
      });
      const raw=await response.text();
      let data:any={};
      try{data=raw?JSON.parse(raw):{};}catch{throw new Error('The job service returned invalid data. Please try again.');}
      ...
      console.warn('Job search:',error);
      setJobs([]);
      setSearchError(error instanceof Error ? error.message : 'Live job discovery is temporarily unavailable.');
  ...
  if(showLanding&&!authenticated)return (
    <div className="min-h-screen bg-[#050505] flex flex-col relative overflow-hidden">
      <AmbientMotion />
      <div className="relative z-10 flex min-h-screen flex-col">
        <LandingPage onGetStarted={openSignUp} onSignIn={openSignIn}/>
        <DotFooter/>
        <AuthModal isOpen={isAuthModalOpen} initialMode={authModalMode} onClose={()=>setIsAuthModalOpen(false)} onSuccess={u=>{setAuthUser(u);setShowLanding(false);setIsAuthModalOpen(false);setActiveTab('onboarding');}}/>
      </div>
    </div>
  );
