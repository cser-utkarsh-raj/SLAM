...
const logos: Record<Provider, { site: string; label: string }> = {
  ...
  remoteok: { site: 'https://remoteok.com/favicon.ico', label: 'Remote OK' },
};
...
const BrandMark: React.FC<LogoProps & { provider: Provider }> = ({ provider, className = 'w-10 h-10', size }) => {
  ...
  return (
    <span className="inline-flex items-center justify-center rounded-xl bg-white border border-white/10 shadow-sm shrink-0 overflow-hidden" style={{ width: px, height: px }}>
      <img src={meta.site} width={px} height={px} className={`${className} object-contain p-1`} alt={`${meta.label} logo`} loading="eager" decoding="async" onError={() => setFailed(true)} />
    </span>
  );
};
...
export const RemoteOKLogo: React.FC<LogoProps> = (props) => <BrandMark provider="remoteok" {...props} />;
