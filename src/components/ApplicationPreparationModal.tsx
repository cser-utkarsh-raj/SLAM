...
import { X, Sparkles, FileText, Mail, CheckCircle2, Copy, Check, ShieldCheck, Briefcase, AlertCircle, ExternalLink } from 'lucide-react';
...
export const ApplicationPreparationModal: React.FC<Props> = ({ job, userProfile, isOpen, onClose, onSaveToTracker }) => {
  ...
  const generateCoverLetter = async () => {
    ...
    try {
      ...
      if (!response.ok || !data?.coverLetter) throw new Error(data?.detail || 'The cover-letter service is unavailable.');
      setCoverLetter(String(data.coverLetter));
    } catch (error) {
      setCoverLetter('');
      setLetterError(error instanceof Error ? error.message : 'No verified cover letter could be generated.');
    } finally {
  ...
  const description = String((job as any).description || '').trim();
  const requirements = ((job as any).requiredSkills || (job as any).requirements || []) as string[];
  ...
  return <div className="fixed inset-0 z-50 bg-[#050505]/95 backdrop-blur-md flex items-center justify-center p-4">
    <div className="bg-[#0a0a0a] border border-zinc-800 w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
      <div className="p-6 border-b border-zinc-800 flex items-start justify-between bg-zinc-950/80"><div><div className="flex items-center gap-2 mb-1"><span className="px-2 py-0.5 bg-yellow-400/10 text-yellow-400 text-[10px] font-mono font-bold tracking-widest uppercase border border-yellow-400/30">Auto Apply</span><span className="text-xs text-zinc-500">Review before leaving SLAM</span></div><h2 className="text-2xl font-display font-black text-white">{job.title}</h2><div className="text-xs text-zinc-400 mt-0.5">{job.company} · {job.location || 'Location not specified'} · {job.primarySource || 'Source not specified'}</div></div><button onClick={onClose} className="p-1.5 text-zinc-500 hover:text-white rounded-lg hover:bg-zinc-800"><X className="w-5 h-5" /></button></div>

      <div className="flex border-b border-zinc-800 bg-zinc-900/50 px-6 gap-2 overflow-x-auto">{[['overview','Job Details',Briefcase],['resume','Resume',FileText],['letter','Cover Letter',Mail],['screening','Screening',ShieldCheck]].map(([id,label,Icon]) => <button key={String(id)} onClick={() => setActiveTab(id as any)} className={`py-3 px-4 text-xs font-bold flex items-center gap-2 border-b-2 whitespace-nowrap ${activeTab === id ? 'border-yellow-400 text-yellow-400 bg-yellow-400/5' : 'border-transparent text-zinc-400'}`}><Icon className="w-3.5 h-3.5" />{label}</button>)}</div>

      <div className="p-6 overflow-y-auto flex-1 space-y-6">
        {activeTab === 'overview' && <div className="space-y-6"><div className="grid grid-cols-2 sm:grid-cols-4 gap-3"><div className="p-3 bg-zinc-900 border border-zinc-800 rounded-lg"><div className="text-[10px] font-mono text-zinc-500 uppercase">Match Score</div><div className="text-2xl font-black text-yellow-400 font-display mt-0.5">{typeof score === 'number' ? `${score}%` : '—'}</div></div><div className="p-3 bg-zinc-900 border border-zinc-800 rounded-lg"><div className="text-[10px] font-mono text-zinc-500 uppercase">Experience</div><div className="text-sm font-bold text-white mt-1">{job.minYearsExperience ? `${job.minYearsExperience}+ years` : 'Not specified'}</div></div><div className="p-3 bg-zinc-900 border border-zinc-800 rounded-lg"><div className="text-[10px] font-mono text-zinc-500 uppercase">Workplace</div><div className="text-sm font-bold text-white mt-1">{job.remote ? 'Remote' : job.location || 'Not specified'}</div></div><div className="p-3 bg-zinc-900 border border-zinc-800 rounded-lg"><div className="text-[10px] font-mono text-zinc-500 uppercase">Source</div><div className="text-sm font-bold text-white mt-1 truncate">{job.primarySource || 'Not specified'}</div></div></div>
          <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-5"><div className="flex items-center justify-between gap-3 mb-3"><h4 className="text-xs font-mono uppercase tracking-wider text-zinc-400 font-bold">Job description</h4><a href={job.applicationUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-yellow-400 font-bold inline-flex items-center gap-1.5">Open original <ExternalLink className="w-3 h-3" /></a></div><p className="text-sm text-zinc-300 leading-7 whitespace-pre-wrap">{description || 'The source did not provide a readable description.'}</p></div>
          {requirements.length > 0 && <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-5"><h4 className="text-xs font-mono uppercase tracking-wider text-zinc-400 font-bold mb-3">Requirements & skills</h4><div className="flex flex-wrap gap-2">{requirements.slice(0,20).map((s) => <span key={s} className="px-2.5 py-1 bg-zinc-900 border border-zinc-700 text-xs text-zinc-200 rounded-md">{s}</span>)}</div></div>}
          <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-5"><h4 className="text-xs font-mono uppercase tracking-wider text-zinc-400 font-bold mb-3">Your skills</h4>{userProfile.skills.length ? <div className="flex flex-wrap gap-2">{userProfile.skills.map((s) => <span key={s} className="px-2.5 py-1 bg-zinc-900 border border-zinc-700 text-xs text-zinc-200 rounded-md flex items-center gap-1.5"><CheckCircle2 className="w-3 h-3 text-emerald-400" />{s}</span>)}</div> : <div className="text-xs text-zinc-500">No skills have been added yet.</div>}</div>
          <div className="p-4 bg-zinc-900/60 border border-zinc-800 rounded-xl"><div className="text-xs font-bold text-white mb-1">How Auto Apply works</div><p className="text-xs text-zinc-400 leading-relaxed">SLAM prepares the information you have provided, saves it to your application tracker, and opens the original application page. It does not bypass CAPTCHA, authentication or employer controls.</p></div>
        </div>}

        {activeTab === 'resume' && <div className="space-y-4"><div className="flex items-center justify-between"><div className="text-xs text-zinc-400">Formatted from the profile information currently stored in SLAM.</div><button onClick={() => void copy(buildResumeText(),'resume')} className="px-3 py-1.5 bg-zinc-800 text-xs font-bold text-white rounded border border-zinc-700 flex items-center gap-1.5">{copiedResume ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5 text-yellow-400" />}{copiedResume ? 'Copied!' : 'Copy Resume'}</button></div><pre className="bg-zinc-950 border border-zinc-800 rounded-lg p-5 font-mono text-xs text-zinc-300 whitespace-pre-wrap leading-relaxed max-h-[420px] overflow-y-auto">{buildResumeText()}</pre></div>}

        {activeTab === 'letter' && <div className="space-y-4"><div className="flex items-center justify-between"><div className="text-xs text-zinc-400">Based on your profile and this listing.</div><div className="flex gap-2"><button onClick={() => void generateCoverLetter()} disabled={isGeneratingLetter} className="px-3 py-1.5 bg-zinc-800 disabled:opacity-50 text-xs font-semibold text-zinc-300 rounded border border-zinc-700 flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5 text-yellow-400" />{isGeneratingLetter ? 'Generating…' : 'Generate with AI'}</button><button onClick={() => void copy(coverLetter,'letter')} disabled={!coverLetter} className="px-3 py-1.5 bg-zinc-800 disabled:opacity-40 text-xs font-bold text-white rounded border border-zinc-700 flex items-center gap-1.5">{copiedLetter ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5 text-yellow-400" />}{copiedLetter ? 'Copied!' : 'Copy'}</button></div></div>{letterError && <div className="p-3 bg-yellow-950/30 border border-yellow-800/40 rounded text-yellow-300 text-xs flex items-start gap-2"><AlertCircle className="w-4 h-4 shrink-0" /><span>{letterError}</span></div>}<textarea rows={12} value={coverLetter} onChange={(e) => setCoverLetter(e.target.value)} placeholder="Generate a cover letter, then review it." className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-4 font-mono text-xs text-zinc-200 focus:outline-none focus:border-yellow-400 leading-relaxed" /></div>}

        {activeTab === 'screening' && <div className="space-y-5"><div className="p-4 bg-yellow-950/20 border border-yellow-900/50 rounded-xl text-xs text-yellow-200">Only use answers supported by the information in your profile.</div><label className="block text-xs text-zinc-400">Work Authorization<input value={authAnswer} onChange={(e) => setAuthAnswer(e.target.value)} placeholder="Not provided" className="mt-1 w-full bg-zinc-950 border border-zinc-800 rounded-lg p-3 text-xs text-white" /></label><label className="block text-xs text-zinc-400">Notice Period / Earliest Start<input value={noticeAnswer} onChange={(e) => setNoticeAnswer(e.target.value)} placeholder="Not provided" className="mt-1 w-full bg-zinc-950 border border-zinc-800 rounded-lg p-3 text-xs text-white" /></label><label className="block text-xs text-zinc-400">Compensation Expectation<input value={salaryAnswer} onChange={(e) => setSalaryAnswer(e.target.value)} placeholder="Not provided" className="mt-1 w-full bg-zinc-950 border border-zinc-800 rounded-lg p-3 text-xs text-white" /></label><label className="block text-xs text-zinc-400">Location & Relocation Preference<input value={relocationAnswer} onChange={(e) => setRelocationAnswer(e.target.value)} placeholder="Not provided" className="mt-1 w-full bg-zinc-950 border border-zinc-800 rounded-lg p-3 text-xs text-white" /></label></div>}
      </div>

      <div className="p-5 border-t border-zinc-800 bg-zinc-950/80 flex items-center justify-between gap-4"><div className="text-[11px] text-zinc-500">Review before continuing to the employer.</div><button onClick={saveAndTrack} className="px-5 py-2.5 bg-yellow-400 hover:bg-yellow-300 text-black text-xs font-black uppercase rounded-lg">Auto Apply</button></div>
    </div>
  </div>;
};
