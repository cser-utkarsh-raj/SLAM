    ...
    <div className="bg-[#0a0a0a] border border-zinc-800 w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
      ...
      <div className="p-6 overflow-y-auto flex-1 space-y-6">
        {activeTab === 'overview' && <div className="space-y-6"><div className="grid grid-cols-2 sm:grid-cols-4 gap-3"><div className="p-3 bg-zinc-900 border border-zinc-800 rounded-lg"><div className="text-[10px] font-mono text-zinc-500 uppercase">Match Score</div><div className="text-2xl font-black text-yellow-400 font-display mt-0.5">{typeof score === 'number' ? `${score}%` : '—'}</div></div><div className="p-3 bg-zinc-900 border border-zinc-800 rounded-lg"><div className="text-[10px] font-mono text-zinc-500 uppercase">Experience</div><div className="text-sm font-bold text-white mt-1">{job.minYearsExperience ? `${job.minYearsExperience}+ years` : 'Not specified'}</div></div><div className="p-3 bg-zinc-900 border border-zinc-800 rounded-lg"><div className="text-[10px] font-mono text-zinc-500 uppercase">Workplace</div><div className="text-sm font-bold text-white mt-1">{job.remote ? 'Remote' : job.location || 'Not specified'}</div></div><div className="p-3 bg-zinc-900 border border-zinc-800 rounded-lg"><div className="text-[10px] font-mono text-zinc-500 uppercase">Source</div><div className="text-sm font-bold text-white mt-1 truncate">{job.primarySource || 'Not specified'}</div></div></div>
          ...
          {requirements.length > 0 && <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-5"><h4 className="text-xs font-mono uppercase tracking-wider text-zinc-400 font-bold mb-3">Detected requirements & skills</h4><div className="flex flex-wrap gap-2">{requirements.slice(0,20).map((s) => <span key={s} className="px-2.5 py-1 bg-zinc-900 border border-zinc-700 text-xs text-zinc-200 rounded-md">{s}</span>)}</div></div>}
          <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-5"><h4 className="text-xs font-mono uppercase tracking-wider text-zinc-400 font-bold mb-3">Verified candidate skills</h4>{userProfile.skills.length ? <div className="flex flex-wrap gap-2">{userProfile.skills.map((s) => <span key={s} className="px-2.5 py-1 bg-zinc-900 border border-zinc-700 text-xs text-zinc-200 rounded-md flex items-center gap-1.5"><CheckCircle2 className="w-3 h-3 text-emerald-400" />{s}</span>)}</div> : <div className="text-xs text-zinc-500">No skills have been verified yet.</div>}</div>
          <div className="p-4 bg-zinc-900/60 border border-zinc-800 rounded-xl"><div className="text-xs font-bold text-white mb-1">What Auto Apply actually does</div><p className="text-xs text-zinc-400 leading-relaxed">SLAM prepares your verified resume, screening answers and an AI cover letter when an AI provider is configured, then opens the original application page. It does not bypass CAPTCHA, authentication or employer controls.</p></div>
        </div>}
        ...
        {activeTab === 'letter' && <div className="space-y-4"><div className="flex items-center justify-between"><div className="text-xs text-zinc-400">Grounded in your verified profile and this listing only.</div><div className="flex gap-2"><button onClick={() => void generateCoverLetter()} disabled={isGeneratingLetter} className="px-3 py-1.5 bg-zinc-800 disabled:opacity-50 text-xs font-semibold text-zinc-300 rounded border border-zinc-700 flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5 text-yellow-400" />{isGeneratingLetter ? 'Generating…' : 'Generate with AI'}</button><button onClick={() => void copy(coverLetter,'letter')} disabled={!coverLetter} className="px-3 py-1.5 bg-zinc-800 disabled:opacity-40 text-xs font-bold text-white rounded border border-zinc-700 flex items-center gap-1.5">{copiedLetter ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5 text-yellow-400" />}{copiedLetter ? 'Copied!' : 'Copy'}</button></div></div>{letterError && <div className="p-3 bg-yellow-950/30 border border-yellow-800/40 rounded text-yellow-300 text-xs flex items-start gap-2"><AlertCircle className="w-4 h-4 shrink-0" /><span>{letterError}</span></div>}<textarea rows={12} value={coverLetter} onChange={(e) => setCoverLetter(e.target.value)} placeholder="Generate a factual cover letter with AI, then review it." className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-4 font-mono text-xs text-zinc-200 focus:outline-none focus:border-yellow-400 leading-relaxed" /></div>}
      ...
      <div className="p-5 border-t border-zinc-800 bg-zinc-950/80 flex items-center justify-between gap-4"><div className="text-[11px] text-zinc-500 font-mono">Review before external submission.</div><button onClick={saveAndTrack} className="px-5 py-2.5 bg-yellow-400 hover:bg-yellow-300 text-black text-xs font-black uppercase rounded-lg">Save & Open Application</button></div>
    </div>
  </div>;
};
