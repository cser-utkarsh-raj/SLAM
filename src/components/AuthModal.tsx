...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onSuccess, initialMode = 'signin' }) => {
  ...
  return (
    <div
      className="fixed inset-0 z-50 bg-[#050505]/90 backdrop-blur-md overflow-y-auto"
      role="dialog"
      aria-modal="true"
      aria-label={mode === 'signup' ? 'Create your SLAM account' : 'Sign in to SLAM'}
    >
      <div className="min-h-[100dvh] w-full flex items-start sm:items-center justify-center p-3 sm:p-6">
        <div className="bg-[#0a0a0a] border border-zinc-800 w-full max-w-md max-h-[calc(100dvh-1.5rem)] sm:max-h-[calc(100dvh-3rem)] overflow-y-auto p-5 sm:p-8 relative shadow-2xl rounded-2xl">
          <button
            onClick={onClose}
            className="absolute top-3 right-3 sm:top-4 sm:right-4 z-20 w-9 h-9 flex items-center justify-center text-zinc-400 hover:text-white bg-zinc-950/80 hover:bg-zinc-900 border border-zinc-800 rounded-lg transition cursor-pointer"
            ...
            {mode === 'signup' && (
              <div>
                ...
                <div className="relative">
                  ...
                  <input required value={name} onChange={(e) => setName(e.target.value)} autoComplete="name" className="w-full bg-zinc-950 border border-zinc-800 rounded-xl pl-9 pr-12 py-2.5 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-yellow-400 transition" placeholder="Your full name" />
                </div>
              </div>
            )}
