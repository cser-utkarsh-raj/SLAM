...
const PLATFORMS = [
  { id: 'arbeitnow', name: 'Arbeitnow', Logo: ArbeitnowLogo, href: SOURCE_URLS.arbeitnow, kind: 'live', desc: 'Live public job feed connected to SLAM' },
    ...
    <div className="max-w-7xl mx-auto px-6">
      ...
      <div className="mb-10 border border-zinc-800 bg-[#090909] rounded-2xl p-5 sm:p-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-5">
          <div><div className="flex items-center gap-2"><Radio className="w-4 h-4 text-emerald-400" /><span className="text-xs font-mono font-bold uppercase tracking-widest text-emerald-400">LIVE PROFILE MATCHING</span></div><h3 className="text-xl sm:text-2xl font-display font-black text-white mt-1">Real jobs matched to your profile.</h3><p className="text-[11px] text-zinc-500 font-mono mt-1">{profile?.country ? `Market: ${profile.country}` : 'Market: profile country required'}{activeQuery ? ` · Query: ${activeQuery}` : ''}</p></div>
          <button type="button" onClick={() => void loadLiveMatches(readProfile())} disabled={loadingJobs} className="px-4 py-2.5 bg-yellow-400 hover:bg-yellow-300 disabled:opacity-50 text-black rounded-xl text-xs font-black uppercase tracking-wider flex items-center justify-center gap-2 transition shrink-0"><RefreshCw className={`w-3.5 h-3.5 ${loadingJobs ? 'animate-spin' : ''}`} />{loadingJobs ? 'FETCHING LIVE DATA' : 'REFRESH LIVE MATCHES'}</button>
        </div>

        {loadingJobs && <div className="py-8 text-center text-xs font-mono text-zinc-500">Querying the live source and calculating compatibility...</div>}

        {!loadingJobs && liveJobs.length > 0 && (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
            {liveJobs.map((job) => {
              const score = job.match?.compatibilityScore;
              const source = job.sourcesList?.[0];
              return (
                <a key={job.id} href={job.applicationUrl} target="_blank" rel="noreferrer" className="block border border-zinc-800 bg-zinc-950 rounded-xl p-4 hover:border-yellow-400/60 hover:bg-zinc-900 transition group">
                  <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="text-sm font-bold text-white group-hover:text-yellow-300 line-clamp-2">{job.title}</div><div className="text-xs text-zinc-400 mt-0.5 truncate">{job.company}</div></div>{typeof score === 'number' && <div className="text-lg font-black font-display text-yellow-400 shrink-0">{score}%</div>}</div>
                  <div className="flex items-center gap-2 mt-3 text-[10px] font-mono text-zinc-500"><MapPin className="w-3 h-3" /><span className="truncate">{job.location || 'Location not specified'}</span></div>
                  <div className="mt-3 flex items-center justify-between gap-2 text-[10px] font-mono"><span className="text-emerald-400 truncate">Source: {source?.sourceName || job.primarySource || 'Unknown'}</span><span className="text-zinc-600 group-hover:text-zinc-300 flex items-center gap-1 shrink-0">OPEN <ExternalLink className="w-3 h-3" /></span></div>
                </a>
              );
            })}
          </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {PLATFORMS.map(({ id, name, Logo, href, kind, desc }) => {
          ...
          return (
            <div key={id} className="border border-zinc-800 bg-[#090909] p-5 rounded-xl hover:border-zinc-700 hover:-translate-y-1 transition-all duration-300">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-4 min-w-0"><Logo size={46} /><div className="min-w-0"><div className="text-sm font-bold text-white font-display">{name}</div><div className="text-[10px] text-zinc-400 leading-snug mt-1">{desc}</div></div></div>
                {live ? <Radio className="w-4 h-4 text-emerald-400 shrink-0 mt-1" /> : <CircleSlash2 className="w-4 h-4 text-zinc-700 shrink-0 mt-1" />}
              </div>
              <div className="mt-5 pt-3 border-t border-zinc-900 flex items-center justify-between gap-3"><span className={`text-[10px] font-mono uppercase tracking-wider ${live ? 'text-emerald-400' : 'text-zinc-500'}`}>{live ? 'LIVE SOURCE' : 'OFFICIAL SITE'}</span><a href={href} target="_blank" rel="noreferrer" className="px-3 py-1.5 border border-zinc-800 hover:border-zinc-600 rounded-lg text-xs font-mono text-zinc-300 hover:text-white transition flex items-center gap-1.5"><span>Visit site</span><ExternalLink className="w-3 h-3 text-zinc-500" /></a></div>
            </div>
          );
        })}
      </div>
