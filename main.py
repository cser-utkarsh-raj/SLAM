...
def ai_clients():
    ...
    if nvidia_key:
        ...
        if nvidia_model.startswith("nvapi-") or "/" not in nvidia_model: nvidia_model = "meta/llama-3.1-70b-instruct"
    ...
    if openrouter_key:
        ...
        if openrouter_model.startswith("sk-or-"): openrouter_model = "openrouter/free"
...
def parse_json_object(text: str):
    ...
    try:
        value = json.loads(text); return value if isinstance(value, dict) else None
    except json.JSONDecodeError:
        match = re.search(r"\{.*\}", text, re.S)
        if not match: return None
        try:
            value = json.loads(match.group(0)); return value if isinstance(value, dict) else None
...
def sanitize_profile(raw: dict, source: str):
    ...
    p["workHistory"] = [w for w in p["workHistory"] if isinstance(w, dict) and (str(w.get("company","")).lower() in src or str(w.get("role","")).lower() in src)]
    return p
...
async def razorpay_request(path:str, method:str="GET", payload:dict|None=None):
    key = get_secret(["RAZORPAY_KEY_ID"]); secret = get_secret(["RAZORPAY_KEY_SECRET", "RAZORPAY_KEY_SECI"])
